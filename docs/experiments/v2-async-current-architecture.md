...
`POST /api/posts`는 게시글을 `imageStatus=PENDING`으로 저장하고 SQS에 작업을 넣은 뒤 바로 응답한다.
...
13. Spring이 게시글의 `imageStatus`를 `COMPLETED` 또는 `FAILED`로 갱신한다.
...
- Spring ↔ MongoDB
  - 게시글 `imageStatus` 저장 / 조회
- Spring ↔ MySQL / Redis
  - 본 이미지 상태 플로우의 주 저장소는 아님
- k6 ↔ Spring
