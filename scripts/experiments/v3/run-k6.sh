...
ensure_file "${PROJECT_ROOT}/test/k6-script/image-pipeline-v2.js"

ssh_run "${K6_HOST}" "sudo mkdir -p ${REMOTE_DIR} && sudo chown -R $(ssh_user):$(ssh_user) ${REMOTE_DIR}"
scp_to "${PROJECT_ROOT}/test/k6-script/image-pipeline-v2.js" "${K6_HOST}" "${REMOTE_DIR}/image-pipeline-v2.js"
